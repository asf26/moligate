...
// AdminSubscriptionOrderQuery filters the admin order ledger. Empty values mean
// "no constraint"; the time bounds are inclusive unix seconds on create_time.
type AdminSubscriptionOrderQuery struct {
	UserId    int
	Username  string
	TradeNo   string
	Status    string
...
// AdminSubscriptionOrderRow is one row of the admin order ledger. The snapshot
// is read to resolve the purchased plan title and is never serialized.
type AdminSubscriptionOrderRow struct {
	Id              int     `json:"id"`
	UserId          int     `json:"user_id"`
...
// ListAdminSubscriptionOrders returns one page of every user's orders, newest
// first, together with the buyer's username and the purchased plan title.
func ListAdminSubscriptionOrders(query AdminSubscriptionOrderQuery, pageInfo *common.PageInfo) ([]AdminSubscriptionOrderRow, int64, error) {
	if pageInfo == nil {
	...
	scoped := func() *gorm.DB {
		tx := DB.Table("subscription_orders AS o").
			Joins("LEFT JOIN users AS u ON u.id = o.user_id")
		if query.UserId > 0 {
			tx = tx.Where("o.user_id = ?", query.UserId)
		}
		...
		if tradeNo := strings.TrimSpace(query.TradeNo); tradeNo != "" {
			tx = tx.Where("o.trade_no LIKE ?", "%"+tradeNo+"%")
		}
		if status := strings.TrimSpace(query.Status); status != "" {
			tx = tx.Where("o.status = ?", status)
		}
		if query.StartTime > 0 {
			tx = tx.Where("o.create_time >= ?", query.StartTime)
		}
		if query.EndTime > 0 {
			tx = tx.Where("o.create_time <= ?", query.EndTime)
		}
		return tx
	}
	...
	rows := make([]AdminSubscriptionOrderRow, 0, pageInfo.PageSize)
	err := scoped().
		Select("o.id, o.user_id, o.plan_id, o.money, o.trade_no, o.payment_method, " +
			"o.payment_provider, o.status, o.create_time, o.complete_time, o.plan_snapshot, u.username").
	...
	for i := range rows {
		if title := subscriptionOrderSnapshotPlanTitle(rows[i].PlanSnapshot); title != "" {
			rows[i].PlanTitle = title
		}
	}
