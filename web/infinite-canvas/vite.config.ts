...
const localChangelog = existsSync(localChangelogFile) ? readFileSync(localChangelogFile, "utf8") : "";
const canvasBase = (process.env.VITE_BASE || "/canvas-app/").replace(/\/+$/, "/");
    ...
    const listLocalPlugins = () => {
        try {
            return readdirSync(pluginsDir)
                ...
                .sort()
                .map((file) => `${canvasBase}plugins/${file}`);
        } catch {
            return [];
        }
    };
...
export default defineConfig({
    base: canvasBase,
    build: {
        outDir: resolve(webDir, "../canvas-public/canvas-app"),
        emptyOutDir: true,
    },
