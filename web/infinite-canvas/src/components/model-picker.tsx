...
function emptyModelLabel(config: AiConfig, capability?: ModelCapability) {
    ...
    if (capability && config.models.length) return i18n.t("settingsPanels.model.noMatch", { capability: label });
...
function resolveModelIcon(model: string) {
    ...
    if (name.includes("claude") || name.includes("anthropic")) return canvasAssetUrl("icons/claude.svg");
    if (name.includes("gemini") || name.includes("google")) return canvasAssetUrl("icons/gemini.svg");
    if (name.includes("gpt") || name.includes("openai")) return canvasAssetUrl("icons/openai.svg");
    if (name.includes("grok") || name.includes("grok")) return canvasAssetUrl("icons/grok.svg");
    if (name.includes("deepseek") || name.includes("deepseek")) return canvasAssetUrl("icons/deepseek.svg");
    if (name.includes("glm") || name.includes("glm")) return canvasAssetUrl("icons/glm.svg");
    return "";
}
