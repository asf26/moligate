    ...
    return (
        <div ref={rootRef} className="pointer-events-none absolute bottom-5 z-50 flex justify-center" style={{ left: 300, right: 16 }}>
            ...
            <div ref={wrapRef} className="thin-scrollbar pointer-events-auto flex h-14 max-w-full items-center gap-1 overflow-x-auto rounded-xl border px-2 shadow-lg backdrop-blur [&>*]:shrink-0" style={dockStyle}>
                ...
                <ToolbarButton id="tool-image" label={t("canvas.toolbar.image")} showLabel hovered={hovered} hoverStyle={hoverStyle} wrapRef={wrapRef} onTipX={setTipX} onHover={setHovered} onClick={onAddImage}>
                    <ImageIcon className="size-4.5" />
                </ToolbarButton>
                <ToolbarButton id="tool-video" label={t("canvas.toolbar.video")} showLabel hovered={hovered} hoverStyle={hoverStyle} wrapRef={wrapRef} onTipX={setTipX} onHover={setHovered} onClick={onAddVideo}>
            ...
            {appearanceOpen ? (
                ...
                >
                    ...
                    <div className="px-1 pb-1.5 text-[11px] font-medium opacity-50">{t("canvas.toolbar.gridStyle")}</div>
                    <Segmented
...
function ToolbarButton({
    ...
    return (
        <Button
            type="text"
            aria-label={label}
            className={`!h-8 !min-w-8 !p-0 ${showLabel ? "!w-auto !gap-1.5 !px-2.5" : "!w-8"}`}
            disabled={disabled}
            ...
            onClick={onClick}
        >
            {showLabel ? <span className="text-xs font-medium leading-none">{label}</span> : null}
        </Button>
    );
}
