...
export function planToFormValues(plan: SubscriptionPlan): PlanFormValues {
  return {
    ...
    sale_enabled: plan.sale_enabled === true,
    ...
    daily_amount: quotaUnitsToDollars(Number(plan.daily_amount || 0)),
    weekly_amount: quotaUnitsToDollars(Number(plan.weekly_amount || 0)),
    monthly_amount: quotaUnitsToDollars(Number(plan.monthly_amount || 0)),
...
export function formValuesToPlanPayload(values: PlanFormValues): PlanPayload {
  return {
    plan: {
      ...
      daily_amount: parseQuotaFromDollars(Number(values.daily_amount || 0)),
