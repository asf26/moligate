    ...
    <>
              ...
              {importUsage && (
                <div className='flex items-center justify-between gap-3'>
                  ...
                  <Button
                    ...
                    size='sm'
                    disabled={
                      previewing || !selectedPlanId || !effectiveStartTime
                    }
                ...
                <div className='grid grid-cols-2 gap-x-4 gap-y-2 rounded-md border px-3 py-2 text-sm sm:grid-cols-4'>
                  {[
                    [
                      ...
                      preview.exceeds_amount_total,
                    ],
                    [
                      t('Daily'),
                      preview.daily_used,
                      preview.daily_amount,
                      preview.exceeds_daily_amount,
                    ],
                    [
                      t('Weekly'),
                      preview.weekly_used,
                      preview.weekly_amount,
                      preview.exceeds_weekly_amount,
                    ],
                    [
                      t('Monthly'),
                      preview.monthly_used,
                      preview.monthly_amount,
                      preview.exceeds_monthly_amount,
                    ],
                    ...
                    <div key={String(label)}>
                      ...
                      >
                        {formatQuota(Number(used))} /{' '}
                        {Number(limit) > 0
                          ? formatQuota(Number(limit))
                          : t('Unlimited')}
                      </div>
                    </div>
                  ))}
                  {!preview.consume_log_enabled && (
                    <p className='text-destructive col-span-full text-xs'>
                      {t(
                        'Consume logging is disabled; imported usage may be incomplete'
                      )}
                    </p>
                  )}
                </div>
      ...
      {confirmOverLimit && preview?.exceeds_any_limit && (
        <ConfirmDialog
          ...
          desc={t(
            'The subscription will be created with exhausted quota in the exceeded periods. Continue?'
          )}
