/*
Copyright (C) 2023-2026 QuantumNous

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU Affero General Public License as
published by the Free Software Foundation, either version 3 of the
License, or (at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU Affero General Public License for more details.

You should have received a copy of the GNU Affero General Public License
along with this program. If not, see <https://www.gnu.org/licenses/>.
*/
import { renderHook } from '@testing-library/react'
import { describe, expect, it } from 'vitest'

import { useSidebarData } from '../use-sidebar-data'

describe('useSidebarData', () => {
  it('places AI Creation after Usage Logs and opens it standalone in a new tab', () => {
    const { result } = renderHook(() => useSidebarData())
    const generalGroup = result.current.navGroups.find(
      (group) => group.id === 'general'
    )
    const generalItems = generalGroup?.items ?? []
    const aiCreationIndex = generalItems.findIndex(
      (item) => 'url' in item && item.title === 'AI Creation'
    )
    const usageLogsIndex = generalItems.findIndex(
      (item) => 'url' in item && item.title === 'Usage Logs'
    )

    expect(aiCreationIndex).toBe(usageLogsIndex + 1)
    expect(generalItems[aiCreationIndex]).toMatchObject({
      title: 'AI Creation',
      url: '/canvas?standalone=true',
      openInNewTab: true,
    })
    expect(generalItems[aiCreationIndex]).toMatchObject({
      activeUrls: ['/canvas', '/canvas/image', '/canvas/video'],
      configUrls: ['/canvas'],
    })

    const chatGroup = result.current.navGroups.find(
      (group) => group.id === 'chat'
    )
    expect(
      chatGroup?.items.some(
        (item) => 'url' in item && item.url?.toString().startsWith('/canvas')
      )
    ).toBe(false)

    const adminGroup = result.current.navGroups.find(
      (group) => group.id === 'admin'
    )
    expect(
      adminGroup?.items.find(
        (item) => 'title' in item && item.title === 'Video Accounts'
      )
    ).toMatchObject({
      title: 'Video Accounts',
      url: '/video-accounts',
    })
  })
})
